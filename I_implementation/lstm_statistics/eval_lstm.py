...
for layer in lstm_model.layers:
    if 'LSTM' in str(layer):
        ...
        plt.ylabel('Rows {}'.format(np.size(weights[0],0)))
        plt.xlabel('Cols {}'.format(np.size(weights[0],1)))
        plt.title('1st weights')
        plt.show()
        
        plt.imshow(weights[1])
        plt.ylabel('Rows {}'.format(np.size(weights[1],0)))
        plt.xlabel('Cols {}'.format(np.size(weights[1],1)))
        plt.title('2nd weights')
        plt.show()
        
        plt.plot(weights[2])
        plt.xlabel('Cols {}'.format(np.size(weights[2],0)))
        plt.title('3rd weights')
        plt.show()
