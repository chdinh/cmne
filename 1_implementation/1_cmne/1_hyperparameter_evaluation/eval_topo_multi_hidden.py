...
# num_epochs = 400 #250 #300
# lstm_look_back = 80 #40 #100 #40
#
# num_units = [[640,640],[1280,1280]]
...
for num_unit in num_units:
    ...
    fitting_result = model.fit_generator(data_generator, steps_per_epoch = steps_per_ep, epochs = num_epochs, verbose=1, validation_data=None, class_weight=None, workers=1)# callbacks=[tbCallBack], validation_data=None, class_weight=None, workers=1)
    ...
    fname_model = settings.repo_path() + 'Results/Models/Model_Opt_3b_' + settings.modality() + '_mh_%s'%'_'.join(map(str, num_unit)) + '_lb_' + str(lstm_look_back) + '_' + date_stamp + '.h5'
    fname_training_loss = settings.repo_path() + 'Results/Training/Loss_Opt_3b_' + settings.modality() + '_mh_%s'%'_'.join(map(str, num_unit)) + '_lb_' + str(lstm_look_back) + '_' + date_stamp + '.txt'
    fname_resultfig = settings.repo_path() + 'Results/img/Loss_Opt_3b_' + settings.modality() + '_mh_%s'%'_'.join(map(str, num_unit)) + '_lb_' + str(lstm_look_back) + '_' + date_stamp + '.png'
...
# save overall plot
date_stamp = datetime.datetime.now().strftime('%Y-%m-%d_%H%M%S')
fname_overall_fig = settings.repo_path() + 'Results/img/Loss_Opt_3b_' + settings.modality() + '_overall_mh_' + date_stamp + '.png'
plt.figure()
plt.xlabel('Minibatch number')
plt.ylabel('Loss')
plt.title('Minibatch run vs. Training loss')
